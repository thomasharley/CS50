...
int main(int argc, char *argv[])
{
    ...
    free(word); /* free up word variable */
    fclose(infile);
}
...
bool check(char *word)
{
    ...
    /* loop checking over every letter in word*/
    for (int i = 0; i < strlen(word); i++) {
            ...
            if (cursor->children[index] != NULL)
            {
                /* set cursor to letter */
                cursor = cursor->children[index];
            }
            else {
                /* next letter in the sequence was not found, so return */
                return false;
            }  
    }
